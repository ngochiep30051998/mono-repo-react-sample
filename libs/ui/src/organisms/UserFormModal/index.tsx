...
interface UserFormModalProps {
  ...
  onClose: () => void;
  userStore?: UserFormStore;
}

export default function UserFormModal({ open, editingId, onClose, userStore }: UserFormModalProps) {
  const { message } = App.useApp();
  const [form] = Form.useForm();

  if (!userStore) return null;
  const { detailLoading, saving, fetchById, create, update, resetDetail } = userStore;
